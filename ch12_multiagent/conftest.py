"""Make the PydanticAI specialists importable offline (see ch06_facade/conftest.py).

`Agent("anthropic:…")` resolves the provider eagerly, which needs the key present
(not validated). Tests override every model with a FunctionModel, so a placeholder
